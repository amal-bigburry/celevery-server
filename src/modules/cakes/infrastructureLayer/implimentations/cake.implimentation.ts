...
export class CakeRepositoryImp implements CakeInterface {
  ...
  async updateCake(
    ...
    let cake = await this.cakeModel.findById(cake_id);
    // console.log(cake, updatecakedto)
    let updatedcake = updateObject(updatecakedto, cake);
    updatedcake.save();
    return 'updated';
  }
  /**
  ...
  async findAvailableCakes(user_id: string): Promise<CakeEntity[]> {
    // getting all the cakes in the platform
    const cakes = await this.cakeModel.find().exec();
    // get the currently available cakes
    const cakeResults = await Promise.all(
      cakes.map(async (cake) => {
        ...
        // check if this cake store is open
        const isOpen = isStoreOpen(openAt, closeAt);
        // check if this cake store is active now
        const isStoreActive = store.is_active;
        // ignore cakes belongs to the requesting customer
        const isNotMine = store.store_owner_id !== user_id;
        // check if the cakes store status is approved. Verifies that the store is not suspended
        const isApproved = store.store_status === STORE_STATUS.APPROVED;
        // check if the cake status is active
        const isCakeActive = cake.is_active;
        // if any of the below conditions fail, that cake is descarded for this request
        console.log(isOpen, isApproved, isStoreActive, isNotMine, isCakeActive)
        return isOpen &&
          isApproved &&
          isStoreActive &&
          isNotMine &&
