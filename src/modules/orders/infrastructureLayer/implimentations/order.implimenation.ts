...
export class OrderRepositoryImp implements OrderInterface {
  ...
  async changeStatusToCancel(data: ChangeOrderStatusDto): Promise<OrderDto> {
    ...
    const buyer = await this.IGetUserDetailUseCase.execute(
      order.buyer_id,
    );
    const seller = await this.IGetUserDetailUseCase.execute(
      order.seller_id,
    );
    let mqttdata: PopDto = {
      topic: seller._id,
      message: `{"type": "cancel_order", "data": "${order_id}"}`,
    };
    ...
    let finalPlacedOrders = (
      await Promise.all(
        orders.map(async (order) => {
          const cake = await this.getCakeDetailsUseCase.execute(
            order.cake_id,
          );
          if (!cake) return null;
