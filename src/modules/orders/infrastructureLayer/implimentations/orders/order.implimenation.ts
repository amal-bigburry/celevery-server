...
import { IGetUserDetailUseCase } from 'src/modules/orders/applicationLayer/interfaces/get-user-details.interface';
import { INotificationUseCase } from 'src/modules/orders/applicationLayer/interfaces/notification.interface';
import { OrderRepository } from 'src/modules/orders/applicationLayer/interfaces/order.interface';
...
import { IMqttService } from 'src/modules/orders/applicationLayer/interfaces/mqtt-service.interface';
...
export class OrderRepositoryImp implements OrderRepository {
  ...
  constructor(
    ...
    private readonly IGetUserDetailUseCase: IGetUserDetailUseCase,
  ...
  async changeStatusToCancel(data: ChangeOrderStatusDto): Promise<OrderDto> {
    ...
    const buyer = await this.IGetUserDetailUseCase.execute(order.buyer_id);
    const seller = await this.IGetUserDetailUseCase.execute(order.seller_id);
    ...
    let finalPlacedOrders = (
      await Promise.all(
        orders.map(async (order) => {
          ...
          const user = await this.IGetUserDetailUseCase.execute(user_id);
          if (!user) return null;
